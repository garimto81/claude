...
def match_by_prefix_number(
    iconik_clips: List[Dict], match_records: List[Dict]
) -> Dict[str, Dict]:
    """접두사 번호로 매칭"""
...
def match_by_players_and_day(
    iconik_clips: List[Dict], match_records: List[Dict], already_matched: set
) -> Dict[str, Dict]:
...
def analyze_and_match(iconik_csv: str, match_csv: str) -> Dict[str, any]:
    ...
    players_day_matches = match_by_players_and_day(
        iconik_clips, match_records, matched_ids
    )
    ...
    print(
        f"  매칭 성공: {len(all_matches)}개 ({len(all_matches)/len(iconik_clips)*100:.1f}%)"
    )
    ...
    if unmatched:
        ...
        for clip in unmatched[:10]:
            ...
            print(
                f"    프로젝트: {clip['project_name']}, 이벤트: {clip['episode_event']}"
            )
    ...
    for i, (iconik_id, match_info) in enumerate(list(all_matches.items())[:10]):
        print(
            f"  [{i+1}] {match_info['match_method']} (신뢰도: {match_info['confidence']:.2f})"
        )
