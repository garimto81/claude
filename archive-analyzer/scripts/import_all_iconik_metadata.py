...
def load_matched_csv(csv_path: str) -> Dict[str, Dict]:
    ...
    with open(csv_path, "r", encoding="utf-8-sig") as f:
        ...
        for row in reader:
            ...
            matches[iconik_id] = {
                "matched_filename": (
                    row.get("matched_filename", "") or row.get("Matched Filename", "")
                ).strip(),
                "matched_file_id": parse_int(
                    row.get("matched_file_id", "") or row.get("Matched File ID", "")
                ),
                "match_method": (
                    row.get("match_method", "") or row.get("Match Method", "")
                ).strip(),
...
def import_all(
    ...
    logger.info(
        f"Import complete: {imported_matched} matched, {imported_unmatched} unmatched"
    )
    return stats
...
def main():
    ...
    parser.add_argument(
        "--matched", "-m", required=True, help="매칭 결과 CSV 파일 경로"
    )
    parser.add_argument(
