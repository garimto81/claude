/**
 * Playwright Base LLM Adapter
 *
 * Electron BrowserView → Playwright Page 전환
...
import type { SelectorSet, ProviderSelectors, AdapterSelectors } from './types';
import { getBaseUrl, getDefaultSelectors, getSelectorSets } from './selector-config';

export class PlaywrightBaseLLMAdapter {
  ...
  constructor(provider: LLMProvider, page: Page) {
  ...
  // ★ 핵심 변경: webContents.executeJavaScript → page.evaluate
  protected async executeScript<T>(script: string, defaultValue?: T): Promise<T> {
    try {
      // Playwright page.evaluate()는 함수나 문자열을 받음
      // 기존 스크립트 문자열을 그대로 사용하기 위해 eval 래퍼 사용
      const result = await this.page.evaluate((scriptStr: string) => {
...
export type { Page, SelectorSet, ProviderSelectors, AdapterSelectors } from './types';
