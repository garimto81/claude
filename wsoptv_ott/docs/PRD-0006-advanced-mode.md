...
| **Layer 3: PlayerCAM** | 각 피처 테이블의 PlayerCAM | n개 (테이블별) |

**핵심 포인트**:
- 모든 피처 테이블이 PlayerCAM을 제공하는 것이 아님 (PlayerCAM 뱃지로 구분)
...
**케이스 3: PlayerCAM이 메인 (PlayerCAM 클릭 시)**
```
┌──────────────────────────────────────────────────────────┐
│ PGM       │                               │              │
│ ┌───────┐ │                               │              │
│ │ 축소  │ │                               │              │
│ │ PGM   │ │    Player 1 PlayerCAM (확대)   │              │
│ └───────┘ │                               │              │
│ 피처테이블│                               │              │
│ ┌───────┐ │                               │              │
│ │Table 1│ │                               │              │
...
| ID | 요구사항 | 설명 | 우선순위 |
|----|---------|------|:--------:|
| MV-1.1 | 3계층 동적 레이아웃 | PGM → 피처 테이블 → PlayerCAM 계층 구조 | P0 |
| MV-1.2 | 레이어 간 동적 전환 | 클릭으로 메인 화면 전환, 레이아웃 재배치 | P0 |
| MV-1.3 | 피처 테이블별 PlayerCAM 활성화 | PlayerCAM 뱃지로 PlayerCAM 지원 여부 표시 | P0 |
...
| **Multi-view** | 멀티앵글 모드 | PGM + PlayerCAM |
...
1. **Multi-view 레이아웃 선호도**
   - PlayerCAM 스타일 (메인 + 하단 9개) vs 4분할 그리드?
   - 제안 배경: 아이돌 PlayerCAM vs 스포츠 멀티뷰 두 가지 방향 존재
