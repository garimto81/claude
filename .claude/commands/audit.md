...
/audit 실행
    ...
    │
    ├─ [Phase 1.7] Prompt Intelligence (자동)
    │       ├─ python .claude/lib/prompt_analyzer.py --stats --json (증분 분석)
    │       ├─ python .claude/lib/prompt_analyzer.py --new-prompts 50 (신규 프롬프트)
    │       ├─ analyst 에이전트 Tier 2 패턴 분석
    │       └─ 결과 캐싱 (.claude/research/audit-prompt-<date>.md)
    │
...
| Phase 1.7 | `/audit` 통합 실행 시만 자동 포함. `config`, `quick` 시 스킵. 증분 분석 (Snapshot+Delta) |
