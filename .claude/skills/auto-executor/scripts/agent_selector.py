...
class AgentSelector:
    ...
    AGENT_PATTERNS: List[Tuple[str, str]] = [
        # 디버깅/오류 관련
        (r"버그|에러|error|실패|fail|디버그|debug|테스트\s*실패|오류|exception", "debugger"),
        ...
        # 백엔드
        (r"API|api|백엔드|backend|서버|server|엔드포인트|endpoint|FastAPI|Django|Express", "backend-dev"),

        # 프론트엔드
        (r"UI|ui|프론트|frontend|컴포넌트|component|화면|React|Vue|Next|CSS|스타일", "frontend-dev"),

        # 보안
        (r"보안|security|취약점|vulnerability|인증|auth|권한|permission|OWASP", "security-auditor"),

        # 문서
        (r"문서|document|README|readme|가이드|guide|설명|docs|API\s*문서", "docs-writer"),

        # 데이터베이스
        (r"DB|db|데이터베이스|database|쿼리|query|마이그레이션|migration|스키마|schema|SQL|Supabase", "database-specialist"),

        # 아키텍처
        (r"설계|design|아키텍처|architecture|구조\s*설계|시스템\s*설계|패턴", "architect"),

        # DevOps
        (r"배포|deploy|CI|CD|인프라|infra|도커|docker|k8s|kubernetes|파이프라인|pipeline", "devops-engineer"),

        # Python
        (r"Python|python|파이썬|FastAPI|Django|pytest|pip|uv|ruff", "python-dev"),

        # TypeScript
        (r"TypeScript|typescript|React|Next|Node|npm|tsx|타입스크립트", "typescript-dev"),

        # Git/커밋
        (r"커밋|commit|git|푸시|push|머지|merge|브랜치|branch", "general-purpose"),

        # 분석/탐색 (기본값과 가까움)
        (r"분석|analyze|탐색|explore|구조|structure|리서치|research|조사|파악", "Explore"),
    ]
