...
**모델 결정**: 에이전트 정의 파일(`.claude/agents/*.md`)의 `model:` 필드가 모델을 결정합니다. Agent() 호출 시 `model` 파라미터는 없습니다.
...
Agent(subagent_type="executor-high", name="impl-manager", description="5조건 자체 루프 구현 관리", team_name="pdca-{feature}", prompt="...")
...
| impl-manager (5조건 루프) | executor-high | opus |
...
Agent(subagent_type="executor", name="jira-runner", description="Jira 조회 실행", team_name="pdca-{feature}",
     prompt="[Jira] --jira {jira_command} {jira_target} 실행.
             ...
             환경변수: ATLASSIAN_EMAIL, ATLASSIAN_API_TOKEN 필수.")
...
if figma_variant == "auth":
    ...
    #   b. HTTP 서버 시작: python -m http.server {port} (npx http-server fallback)
    #   c. 브라우저 열기:
    #      start "" "http://localhost:{port}/{page}#figmacapture={captureId}
...
if "--eco" in options:
    # 모든 Task() 호출의 model 파라미터를 "sonnet"으로 오버라이드
...
```
Agent(subagent_type="executor-high", name="impl-manager", description="5조건 자체 루프 구현 관리", team_name="pdca-{feature}",
...
while cycle < max_cycles:
  ...
  if QA_FAILED:
    ...
    Agent(subagent_type="architect", name="diagnostician-{cycle}", description="진단 분석",
         team_name="pdca-{feature}",
