...
| 5 | **사용자 명시** | `HEAVY` 또는 `상세 계획` 키워드 포함 | 키워드 없음 |
...
if "VERDICT: APPROVE" in first_line:
    ...
    critic_revision_count += 1  # 초기값 0
    if critic_revision_count <= 2:
        → Planner 수정 (critic_feedback = REVISE 사유 전달)
        → Critic-Lite 재검토 (QG1-QG5, max 2회)
    else:
        → 수정본 수용 (2회 수정 후 추가 반복 없음)
else:
    → REVISE로 간주
```

**QG5 (실현 가능성)**: "계획의 각 태스크가 현재 코드베이스에서 실현 가능한가?" — 존재하지 않는 API, 미설치 라이브러리, 불가능한 경로 참조 등을 검출.
...
Task(subagent_type="architect", name="impl-verifier", team_name="pdca-{feature}",
     ...
     prompt="[Phase 2 Architect Gate] 구현 외부 검증.
             ...
             MATCH_RATE: {N}%
             MISSING_ITEMS:
             - [항목번호] {미구현 항목 설명} (파일: {경로})

             산출 방법:
             1. Plan 파일의 구현 항목을 번호 매겨 열거
             2. 각 항목의 실제 구현 여부를 Grep/Read로 검증
             3. MATCH_RATE = (구현 항목 수 / 전체 항목 수) × 100
             4. MATCH_RATE >= 90% → APPROVE, < 90% → REJECT
             5. MISSING_ITEMS: 미구현 항목 목록 (APPROVE 시에도 100% 아니면 출력)")
...
```
[Phase 2 BUILD] Implementation Manager - 5조건 자체 루프
...
if mode == "LIGHT":
    Task(subagent_type="qa-tester", name="qa-runner", team_name="pdca-{feature}",
         model="sonnet", prompt="[Phase 3 QA Runner] 6종 QA 실행. (LIGHT 모드)")
...
while cycle < max_cycles:
  ...
  if QA_FAILED:
    ...
    Task(subagent_type="architect", name="diagnostician-{cycle}",
         team_name="pdca-{feature}", model="opus",
         prompt="[Phase 3 Architect Diagnostician] QA 실패 Root Cause 분석.
...
# Step 3.2.2: Architect 최종 검증 (포그라운드) — 정량적 갭 분석
Task(subagent_type="architect", name="verifier", team_name="pdca-{feature}",
     model="opus",
     prompt="[Phase 3 최종 검증] Plan 대비 구현 일치 정량 분석.
             Plan: docs/01-plan/{feature}.plan.md
             ...
             4. MATCH_RATE >= 90% → APPROVE, < 90% → REJECT
             5. MISSING_ITEMS: 미구현 항목 목록 (APPROVE 시에도 100% 아니면 출력)")
SendMessage(type="message", recipient="verifier", content="정량 검증 시작. MATCH_RATE(%) + MISSING_ITEMS 포함 판정.")
# verifier 완료 대기 → shutdown_request
...
### Phase 3↔4 루프 가드 (모드별 차등)

```
phase3_reentry_count = 0  # Lead 메모리에서 관리
prev_match_rate = 0        # MATCH_RATE 정체 감지용
stagnation_count = 0       # 연속 미변동 횟수

# 모드별 최대 재진입 횟수
MAX_PHASE3_REENTRY = 3 if mode == "STANDARD" else 5  # HEAVY=5 (원본 pdca-iterator 동일)

# Phase 4 → Phase 3 재진입 시
phase3_reentry_count += 1

# MATCH_RATE 정체 감지 (2회 연속 미변동 → 조기 종료)
current_match_rate = Architect VERDICT에서 추출한 MATCH_RATE
if current_match_rate == prev_match_rate:
    stagnation_count += 1
else:
    stagnation_count = 0
prev_match_rate = current_match_rate

if stagnation_count >= 2:
    → "[Phase 4] MATCH_RATE {current_match_rate}%에서 2회 연속 정체. 추가 반복 무의미." 출력
    → 조기 종료 (아래 초과 시 동일 처리)
...
MAX_CUMULATIVE_ITERATIONS = 5 if mode == "STANDARD" else 8  # HEAVY=8

# executor 수정 실행 시 (MISSING_ITEMS 기반 수정)
...
**Case 1: MATCH_RATE < 90% (Architect REJECT + MISSING_ITEMS)**
```
# Architect VERDICT에서 MISSING_ITEMS 추출
missing_items = Architect 응답에서 MISSING_ITEMS 섹션 파싱

Task(subagent_type="executor", name="iterator", team_name="pdca-{feature}",
     model="opus",
     prompt="[Gap Improvement] Architect가 식별한 미구현 항목을 수정하세요.
             MISSING_ITEMS:
             {missing_items}
             각 항목을 순서대로 구현하고, 완료 후 빌드+테스트 통과를 확인하세요.")
SendMessage(type="message", recipient="iterator", content="MISSING_ITEMS 기반 갭 수정 시작.")
# 완료 대기 → shutdown_request → Architect 재검증 (MATCH_RATE 재계산) → Phase 3 재실행
```
...
- Phase 2 STD/HEAVY: Skill(ralph) → impl-manager 5조건 자체 루프 (max 10 iter)
