...
#### `--mockup [파일]` — 3-Tier 자동 목업 생성

정본: `mockup-hybrid/SKILL.md` v2.0. 3-Tier 라우팅(Mermaid/HTML/Stitch)이 출력 형식을 자동 결정.

**Mode A: 문서 기반 (`--mockup docs/02-design/auth.design.md`)**
```
# Step 1: 문서 스캔 → 시각화 필요 섹션 분류 (NEED/SKIP/EXIST)
target_doc = options.get("mockup")  # 문서 경로
# Read(target_doc) → 헤딩 기반 섹션 분리 → 키워드 매칭으로 분류

# Step 2: NEED 섹션 → 3-Tier 라우팅 (각 섹션별)
#   다이어그램 키워드 → Mermaid → .mermaid.md (인라인 코드 블록으로 삽입)
#   UI/화면 키워드 → designer 에이전트 → HTML + PNG
#   고품질 키워드 → Stitch API → HTML + PNG (HiFi)

# Step 3: UI 목업 (HTML 선택 시) → designer 에이전트 위임
Task(subagent_type="designer", name="mockup-designer", team_name="pdca-{feature}",
     model="sonnet",
     prompt="[Mockup] {섹션 내용} 기반 HTML 와이어프레임 생성.
             출력: docs/mockups/{name}.html
             {bnw_constraint}  # --bnw 시 B&W 팔레트 제약 주입
             max-width: 720px, max-height: 1280px")
SendMessage(type="message", recipient="mockup-designer", content="목업 생성 시작.")
# 완료 대기 → shutdown_request
```
...
```
# 3-Tier 라우팅 (키워드 기반)
#   다이어그램 키워드 → Lead가 직접 Mermaid 생성
#   UI/화면 키워드 → designer 에이전트 위임 (위 Step 3과 동일)
#   고품질 키워드 → Stitch API (API 불가 시 HTML 폴백)
```

**Step 2.0.2: PNG 캡처 (designer 완료 후, Lead 직접 실행)**
```bash
python -c "
...
**Step 2.0.3: 문서 삽입 (대상 문서가 있는 경우만, Lead 직접 Edit)**
...
- HTML 선택 시: B&W 팔레트 (#000~#fff), 아이콘 없음, 독창적 타이포그래피

#### `--gdocs` — Google Docs PRD 동기화

```
# prd-sync 커맨드 실행 (Google Docs → 로컬 동기화)
Task(subagent_type="executor", name="prd-syncer", team_name="pdca-{feature}",
     model="opus",
...
Task(subagent_type="executor", name="debate-runner", team_name="pdca-{feature}",
     model="opus",
...
Task(subagent_type="researcher", name="researcher", team_name="pdca-{feature}",
     model="sonnet",
...
Task(subagent_type="executor", name="daily-runner", team_name="pdca-{feature}",
     model="opus",
