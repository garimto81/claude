...
from providers.base import AuthToken, BaseProvider
from auth.browser_oauth import BrowserOAuth, OAuthConfig, OAuthCallbackError
...
class OpenAIProvider(BaseProvider):
    ...
    Browser-based OAuth 2.0 + PKCE 사용.
    ...
    def __init__(self, client_id: Optional[str] = None):
        ...
        self._pending_oauth: Optional[BrowserOAuth] = None  # 2단계 인증용
    ...
    def get_auth_url(self) -> str:
        ...
        """
        from auth.browser_oauth import generate_pkce_challenge
        import secrets
    ...
    async def login(self, manual_mode: bool = False, **kwargs) -> AuthToken:
        """Browser OAuth로 로그인.

        OpenAI OAuth는 두 가지 모드를 지원합니다:
        1. 자동 모드: localhost callback 서버로 자동 수신 (실패 시 수동 모드로 전환)
        2. 수동 모드: 브라우저에서 리디렉션된 URL을 직접 입력

        Args:
            manual_mode: True면 처음부터 수동 모드로 시작
        ...
        """
        # Codex CLI 호환 redirect_uri: /auth/callback
        config = OAuthConfig(
            client_id=self.client_id,
        ...
        # 수동 모드 또는 자동 모드
        if manual_mode:
            oauth = BrowserOAuth(config, fixed_port=self.REDIRECT_PORT, manual_callback=True)
            try:
                token_response = await oauth.authenticate()
            except OAuthCallbackError as e:
                raise ValueError(f"OpenAI 인증 실패: {e}")
        else:
            # 자동 모드 시도 → 실패 시 수동 모드로 전환
            oauth = BrowserOAuth(config, fixed_port=self.REDIRECT_PORT)

            try:
                token_response = await oauth.authenticate(timeout=60)  # 1분 대기
            except OAuthCallbackError as e:
                # 자동 콜백 실패 → 수동 모드로 전환
                from rich.console import Console
                console = Console()
                console.print()
                console.print("[yellow]자동 콜백 수신 실패. 수동 모드로 전환합니다.[/yellow]")
                console.print("[dim]OpenAI OAuth는 공식 앱에서만 자동 콜백을 지원합니다.[/dim]")
                console.print()

                # 수동 모드로 재시도
                oauth_manual = BrowserOAuth(config, fixed_port=self.REDIRECT_PORT, manual_callback=True)
                try:
                    token_response = await oauth_manual.authenticate()
                except OAuthCallbackError as e2:
                    raise ValueError(f"OpenAI 인증 실패: {e2}")
