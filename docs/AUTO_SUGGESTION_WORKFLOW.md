...
자율 판단 기반 완전 자동화 워크플로우 가이드입니다.

**Version**: 2.0.0 | **Date**: 2025-12-30
...
| 파일 | 설명 |
|------|------|
| `.claude/commands/auto.md` | /auto 커맨드 정의 |
| `.claude/workflow/auto_analyzer.py` | 상태 분석 및 작업 판단 (핵심) |
| `.claude/workflow/auto_controller.py` | 상태 관리 및 루프 제어 |
| `.claude/workflow/auto_recovery.py` | 실패 자동 복구 |
| `.claude/workflow/auto_report.py` | 완료 리포트 생성 |
| `.claude/workflow/auto_state.json` | 상태 저장 파일 |
...
| 버전 | 날짜 | 변경 내용 |
|------|------|----------|
| 2.0.0 | 2025-12-30 | 자율 판단 방식으로 재설계 (AI가 다음 작업 판단) |
| 1.2.0 | 2025-12-30 | 핵심 로직 구현 (controller, recovery, report) |
