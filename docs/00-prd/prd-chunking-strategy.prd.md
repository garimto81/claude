...
**버전**: 2.0.0 | **작성일**: 2026-02-19 | **작성자**: Claude
...
**R10. 자동 전략 선택 시 사용자 확인 플로우 (P1)**
`--strategy` 옵션을 명시하지 않은 자동 모드(`auto`)로 실행 시,
청킹 실행 전에 분석 결과를 출력하고 사용자 확인을 요청한다.
...
사용자 선택 옵션:
1. 승인 — 추천 전략으로 청킹 실행
2. 전략 변경 — fixed / hierarchical / semantic 중 선택
3. 취소 — 실행 중단

`--yes` 플래그 또는 `--strategy` 명시 시 확인 단계 스킵 (자동화 환경 지원).
...
- v2.0.0 (2026-02-24): R10 사용자 판단 기반 확인 플로우 추가 — 자동 전략 선택 시 인터랙티브 확인 단계
