# Response Style Guide

비개발자 응답 모드 및 응답 스타일 가이드입니다.

**참조**: `CLAUDE.md` 기본 규칙

---

## 비개발자 응답 모드

**목적**: 로직 흐름 이해 + 실수 가능성 검토

### 숨김 처리 대상

| 숨김 | 표시 |
|------|------|
| 코드 블록 | 변경 파일명 + 줄 수 |
| Edit 도구 결과 | "파일 수정 완료" 한 줄 |
| Bash 실행 상세 | 성공/실패 결과만 |

### 필수 표시 대상

| 항목 | 형식 |
|------|------|
| **로직 흐름** | ASCII 다이어그램 (직접 텍스트) |
| **실수 가능성** | 주의점/리스크 목록 |
| **검증 방법** | 확인할 수 있는 명령어/URL |

### 시각화 규칙

| 출력 위치 | 형식 |
|-----------|------|
| **터미널** | ASCII 다이어그램 직접 출력 (최대 65자 너비) |
| **문서** | Mermaid 코드 블록 |

### 다이어그램 너비 제한

| 항목 | 규칙 |
|------|------|
| **최대 너비** | 65자 (터미널 80자 기준 여백 포함) |
| **긴 텍스트** | 줄바꿈 또는 약어 사용 |
| **박스 내용** | 최대 15자, 초과 시 ... 처리 |

### 터미널 출력 금지 항목

| 금지 항목 | 이유 | 대체 |
|-----------|------|------|
| 이모지 (⚠️💡🔴) | Wide char 너비 계산 오류 | [주의], [팁] 등 텍스트 |
| 마크다운 볼드 (**) | 렌더링 시 너비 불일치 | 대문자 또는 [] 강조 |
| 유니코드 특수문자 | 인코딩 문제 | ASCII 문자만 사용 |

### 응답 구조

```
## 작업 내용
> 한 줄 요약

## 로직 흐름
[ASCII 다이어그램]

## 주의점
- 실수 가능성 1
- 실수 가능성 2

## 변경 파일
| 파일 | 변경 |
|------|------|
| 파일명 | +N, -N |

## 검증
- 명령어: `...`
```

---

## 응답 스타일

작업 완료 시 아래 형식으로 마무리:

```
---
## 요약
> 사용자 질문/요청 내용 1줄 요약

## 답변 정리
| 항목 | 내용 |
|------|------|
| 수행 작업 | ... |
| 변경 파일 | ... |
| 결과 | ... |

## 검증 방법
- 파일: `경로/파일명` (Read로 확인)
- 명령어: `실행 명령어`
- URL: http://localhost:포트 (해당 시)
```

---

## 버전 이력

| 버전 | 날짜 | 변경 |
|------|------|------|
| 1.0.0 | 2025-12-16 | CLAUDE.md에서 분리 (PRD-0034) |
